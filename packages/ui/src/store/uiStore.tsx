...
function uiReducer(state: UIState, action: UIAction): UIState {
  switch (action.type) {
    ...
    case "SET_MEETING":
      return {
        ...state,
        simulation: { ...state.simulation, meeting: action.meeting },
      };
    case "SET_FAST_TRACK":
      return {
        ...state,
        simulation: { ...state.simulation, fastTrackEnabled: action.enabled },
      };
...
const UIStoreContext = createContext<UIStoreContextValue | undefined>(
  undefined,
);
...
export function UIStoreProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const onMeeting = (event: Event) => {
      dispatch({
        type: "SET_MEETING",
        meeting: (event as CustomEvent).detail || null,
      });
    };
  ...
  const contextValue = useMemo<UIStoreContextValue>(() => {
    const selectedAgent = state.selectedAgentId
      ? state.agents[state.selectedAgentId] || null
      : null;
    const filteredTasks = state.tasks.filter((task) => {
      if (
        state.filters.taskStatus !== "all" &&
        task.status !== state.filters.taskStatus
      )
        return false;
      if (
    ...
    return {
      ...
      actions: {
        ...
        setPanelVisibility: (panel, visible) =>
          dispatch({ type: "SET_PANEL_VISIBILITY", panel, visible }),
