...
export class OfficeScene extends Phaser.Scene {
  ...
  private readonly zoneBounds: Record<
    string,
  ...
  private layoutItemsByZone: Record<
    string,
    Array<{
      id: string;
      type: string;
  ...
  private agentSelectionRing: Map<string, Phaser.GameObjects.Graphics> =
    new Map();
  private agentUiSnapshot: Map<
    string,
    { name: string; action: string; status: string }
  > = new Map();
  ...
  create() {
    try {
      ...
      this.statusText = this.add.text(10, 10, "Colyseus Sync: Connecting...", {
        color: "#ffffaa",
        fontSize: "14px",
      });
      ...
      for (let i = 0; i <= 5; i++) {
        ...
        anims.create({
          ...
          frames: anims.generateFrameNumbers(charKey, { start: 0, end: 2 }),
          frameRate: 8,
          repeat: -1,
        });
        anims.create({
          key: `${charKey}-walk-up`,
          frames: anims.generateFrameNumbers(charKey, { start: 7, end: 9 }),
          frameRate: 8,
          repeat: -1,
        });
        anims.create({
          key: `${charKey}-walk-right`,
          frames: anims.generateFrameNumbers(charKey, { start: 14, end: 16 }),
          frameRate: 8,
          repeat: -1,
        });
        hasAnims = true;
      ...
      this.onFocusAgentRequest = (event: Event) => {
        ...
        const byName =
          typeof detail?.agentName === "string" ? detail.agentName : "";
        const targetId =
          byId || (byName ? this.agentNameToSessionId.get(byName) : undefined);
        if (targetId) this.focusAgentTemporarily(targetId);
      };
      eventBus.addEventListener(
        "focus-agent-request",
        this.onFocusAgentRequest,
      );

      this.input.on("pointermove", (pointer: Phaser.Input.Pointer) => {
        if (!this.layoutEditMode || !this.layoutDragItemId || !pointer.isDown)
          return;
        const bounds = this.zoneBounds[this.activeZone];
        const gx = Phaser.Math.Clamp(
          Math.round(pointer.worldX / 16),
          bounds.minX,
          bounds.maxX,
        );
        const gy = Phaser.Math.Clamp(
          Math.round(pointer.worldY / 16),
          bounds.minY,
          bounds.maxY,
        );
        this.layoutItemsByZone[this.activeZone] = this.getActiveZoneItems().map(
          (item) =>
            item.id === this.layoutDragItemId
              ? { ...item, x: gx, y: gy }
              : item,
        );
        this.renderCustomLayout(this.getActiveZoneItems());
        eventBus.dispatchEvent(
          new CustomEvent("layout-item-moved", {
            detail: {
  ...
  async connectToServer() {
    try {
      ...
      this.statusText
        .setText(`Colyseus Sync: Connecting to ${wsEndpoint}...`)
        .setColor("#ffffaa");
      ...
      this.statusText
        .setText("Colyseus Sync: Connected (Waiting for state...)")
        .setColor("#aaffaa");
      ...
      this.room.onStateChange.once((state: any) => {
        ...
        this.room!.onMessage("task-update", (message: any) => {
          eventBus.dispatchEvent(
            new CustomEvent("task-update", { detail: message }),
          );
        });
        this.room!.onMessage("tasks-sync", (message: any) => {
          eventBus.dispatchEvent(
            new CustomEvent("tasks-sync", { detail: message }),
          );
        });
        this.room!.onMessage("highlight-event", (message: any) => {
          eventBus.dispatchEvent(
            new CustomEvent("highlight-event", { detail: message }),
          );
        ...
        this.room!.onMessage("scenario-event", (message: any) => {
          eventBus.dispatchEvent(
            new CustomEvent("scenario-event", { detail: message }),
          );
        });
        this.room!.onMessage("relationship-update", (message: any) => {
          eventBus.dispatchEvent(
            new CustomEvent("relationship-update", { detail: message }),
          );
        });
        this.room!.onMessage("approvals-sync", (message: any) => {
          eventBus.dispatchEvent(
            new CustomEvent("approvals-sync", { detail: message }),
          );
        });
        this.room!.onMessage("agent-roster-sync", (message: any) => {
          eventBus.dispatchEvent(
            new CustomEvent("agent-roster-sync", { detail: message }),
          );
        });
        this.room!.onMessage("meeting-state", (message: any) => {
          eventBus.dispatchEvent(
            new CustomEvent("meeting-state", { detail: message }),
          );
        });
        this.room!.onMessage("fast-track-state", (message: any) => {
          eventBus.dispatchEvent(
            new CustomEvent("fast-track-state", { detail: message }),
          );
        });
        this.room!.onMessage("runtime:queue-state", (message: any) => {
          eventBus.dispatchEvent(
            new CustomEvent("runtime:queue-state", { detail: message }),
          );
        });
        this.room!.onMessage("runtime:run-state", (message: any) => {
          eventBus.dispatchEvent(
            new CustomEvent("runtime:run-state", { detail: message }),
          );
        });
        this.room!.onMessage("runtime:runs-sync", (message: any) => {
          eventBus.dispatchEvent(
            new CustomEvent("runtime:runs-sync", { detail: message }),
          );
        });
        this.room!.onMessage("completed-work-sync", (message: any) => {
          eventBus.dispatchEvent(
            new CustomEvent("completed-work-sync", { detail: message }),
          );
        });
        this.room!.onMessage("layout-sync", (message: any) => {
          this.layoutItemsByZone = this.normalizeLayoutByZone(
            message?.layoutByZone ?? message?.layout,
          );
          this.setActiveZone(message?.zoneId || this.activeZone, false);
          eventBus.dispatchEvent(
            new CustomEvent("layout-sync", {
              detail: {
                items: this.getActiveZoneItems(),
        ...
        this.room!.send("runtime:subscribe", {});

        state.agents.onAdd((agent: AgentState, sessionId: string) => {
          console.log(
            `[Colyseus] Agent added: ${agent.name} at (${agent.x}, ${agent.y})`,
          );
          const container = this.add.container(agent.x * 16, agent.y * 16);

          let sprite;
          const { charKey, labelColor } = getAgentStyle(
          ...
          container.on("pointerdown", () => {
            const nextSelected =
              this.selectedAgentId === sessionId ? null : sessionId;
            this.setSelectedAgent(nextSelected, "pointer");
            eventBus.dispatchEvent(
              new CustomEvent("agent-focus", {
                detail: nextSelected
                  ? { name: agent.name, id: sessionId }
                  : null,
              }),
            );
          });
          ...
          eventBus.dispatchEvent(
            new CustomEvent("agent-state-sync", {
              detail: {
                id: sessionId,
                role: agent.id,
                name: agent.name,
          ...
          agent.onChange(() => {
            ...
            if (sprite.type === "Sprite") {
              ...
              if (agent.x > prevX) {
                s.play(`${charKey}-walk-right`, true);
                s.setFlipX(false);
              } else if (agent.x < prevX) {
                s.play(`${charKey}-walk-right`, true);
                s.setFlipX(true);
              } else if (agent.y > prevY) {
                s.play(`${charKey}-walk-down`, true);
              } else if (agent.y < prevY) {
                s.play(`${charKey}-walk-up`, true);
              } else {
                s.stop();
              }
            }
            ...
            const meaningfulThought =
              thought.length >= 4 &&
              thought !== lastThoughtShown &&
              now - lastThoughtAt > 7000;
            ...
            if (
              (actionChanged || meaningfulThought) &&
            ...
            eventBus.dispatchEvent(
              new CustomEvent("agent-telemetry", {
                detail: {
                  id: sessionId,
                  name: agent.name,
                  mood: Number(agent.mood || 0),
        ...
        state.agents.onRemove((agent: AgentState, sessionId: string) => {
          ...
          eventBus.dispatchEvent(
            new CustomEvent("agent-removed", {
              detail: { id: sessionId, role: agent.id, name: agent.name },
            }),
          );
        });
      });
      ...
      const wsEndpoint = resolveWsEndpoint();
      this.statusText
        .setText(`Colyseus Sync: Failed (${wsEndpoint})`)
        .setColor("#ffaaaa");
    }
  }
  ...
  private emitAgentUiEvent(
    ...
    const snapshot = payload.agentId
      ? this.agentUiSnapshot.get(payload.agentId)
      : undefined;
  ...
  private setSelectedAgent(
    agentId: string | null,
    source: "pointer" | "system" = "pointer",
  ) {
    this.selectedAgentId = agentId;
    this.selectedFollowTarget = agentId
      ? (this.agentSprites.get(agentId) ?? null)
      : null;
  ...
  update() {
    ...
    if (manualPan) {
      ...
      this.cinematicFollowTarget = null;
      this.cinematicReleaseAt = 0;
      if (this.cursors?.left.isDown || this.heldMoveKeys.has("left"))
        this.cameras.main.scrollX -= speed;
      if (this.cursors?.right.isDown || this.heldMoveKeys.has("right"))
        this.cameras.main.scrollX += speed;
      if (this.cursors?.up.isDown || this.heldMoveKeys.has("up"))
        this.cameras.main.scrollY -= speed;
      if (this.cursors?.down.isDown || this.heldMoveKeys.has("down"))
        this.cameras.main.scrollY += speed;
    }
  ...
  private setActiveZone(zoneId: string, focusCamera = false) {
    ...
    this.renderCustomLayout(this.getActiveZoneItems());
    eventBus.dispatchEvent(
      new CustomEvent("zone-state", { detail: { zoneId: this.activeZone } }),
    );
    if (focusCamera) {
      const bounds = this.zoneBounds[this.activeZone];
  ...
  private renderCustomLayout(
    ...
    for (let index = 0; index < items.length; index++) {
      ...
      if (item.label) {
        const label = this.add
          .text(0, 10, item.label.slice(0, 8), {
            fontSize: "8px",
            color: "#dfe6f3",
          })
          .setOrigin(0.5, 0);
      ...
      group.setInteractive(
        new Phaser.Geom.Rectangle(-11, -11, 22, 22),
