...
import { TaskRunRuntime } from "./runtime/TaskRunRuntime";
import {
  getActiveProject,
...
let taskRunRuntime: TaskRunRuntime | null = null;
...
const upsertArtifactFromPath = async (
  ...
  const status: ArtifactStatus =
    context?.status || (parsed.approved && exists ? "validated" : "rejected");
...
app.get("/api/workspace-files", async (_req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ ok: false, error: "Unable to list workspace files." });
  }
});
...
app.get("/api/completed-work", async (_req, res) => {
  try {
    const workspaceRoot = getActiveWorkspaceRoot();
    await mkdir(path.join(workspaceRoot, "completed-work"), {
      recursive: true,
    });
    const files = await listWorkspaceFiles(
      path.join(workspaceRoot, "completed-work"),
    );
    res.json({
      ok: true,
    ...
    res
      .status(500)
      .json({ ok: false, error: "Unable to list completed work files." });
  }
});
...
app.post("/api/projects/start", async (req, res) => {
  try {
    ...
    res
      .status(400)
      .json({ ok: false, error: error?.message || "Unable to start project." });
  }
});
...
app.post("/api/task-runs", async (req, res) => {
  try {
    ...
    const projectId = String(
      req.body?.projectId || req.body?.project || getActiveProject() || "",
    ).trim();
    if (!projectId) {
    ...
    const created = await taskRunRuntime.createTaskRun({
      projectId,
      title,
      requestedBy: String(req.body?.requestedBy || "api"),
      brief: typeof req.body?.brief === "string" ? req.body.brief : undefined,
      acceptanceCriteria: Array.isArray(req.body?.acceptanceCriteria)
        ? req.body.acceptanceCriteria.map((item: unknown) => String(item))
        : undefined,
      context:
        req.body?.context && typeof req.body.context === "object"
          ...
          : undefined,
      parentTaskRunId:
        typeof req.body?.parentTaskRunId === "string"
          ? req.body.parentTaskRunId
          : undefined,
    });
    res.status(201).json({ ok: true, run: created });
  } catch (error: any) {
    res
      .status(400)
      .json({
        ok: false,
        error: error?.message || "Unable to create task run.",
      });
  }
});
...
app.get("/api/task-runs", async (req, res) => {
  try {
    ...
    const runs = await taskRunRuntime.listRuns({
      projectId:
    ...
    res
      ...
      .json({
        ok: false,
        error: error?.message || "Unable to list task runs.",
      });
  }
});
...
app.post("/api/task-runs/:runId/cancel", async (req, res) => {
  try {
    ...
    if (!run) {
      res.status(404).json({ ok: false, error: "Task run not found." });
      return;
    }
    ...
    res
      ...
      .json({
        ok: false,
        error: error?.message || "Unable to cancel task run.",
      });
  }
});
...
app.post("/api/workspace-files/upload", async (req, res) => {
  try {
    ...
    res
      ...
      .json({
        ok: false,
        error: error?.message || "Unable to upload workspace file.",
      });
  }
});
...
app.get("/api/artifacts", async (req, res) => {
  try {
    const existsFilter =
      typeof req.query.exists_on_disk === "string"
    ...
    const artifacts = await memoryStore.listArtifacts({
      ...
      status:
        typeof req.query.status === "string"
          ? (req.query.status as ArtifactStatus)
          : undefined,
    ...
    res
      ...
      .json({
        ok: false,
        error: error?.message || "Unable to list artifacts.",
      });
  }
});
...
app.get("/api/projects/:projectId/artifacts", async (req, res) => {
  try {
    ...
    res
      ...
      .json({
        ok: false,
        error: error?.message || "Unable to list project artifacts.",
      });
  }
});
...
const bootstrap = async () => {
  ...
  const extensionRegistry = await ExtensionRegistry.loadFromFolder(
    configuredExtensionFolder,
  );
  ...
  console.log(
    `[Extensions] Active hooks: ${extensionRegistry.list().join(", ") || "none"}`,
  );

  taskRunRuntime = new TaskRunRuntime(
    memoryStore,
    {
      onQueueState: (payload) => {
        const room = OfficeRoom.getActiveRoom();
        if (room) room.broadcastRuntimeQueueState(payload);
      },
      onRunState: (payload) => {
        const room = OfficeRoom.getActiveRoom();
        if (room) room.broadcastRuntimeRunState(payload);
      },
    },
    {
      poolSize: Number(process.env.TASK_RUN_WORKER_POOL_SIZE || 2),
      maxIterations: Number(process.env.TASK_RUN_MAX_ITERATIONS || 8),
    },
  );
  await taskRunRuntime.initialize();
  taskRunRuntime.start();
  ...
  console.log(
    `[Server] AgentOffice Engine listening on ws://localhost:${PORT}`,
  );
};
