...
export class OfficeRoom extends Room<OfficeState> {
  ...
  private static artifactWriteLogger:
    | ((entry: {
        relativePath: string;
        absolutePath: string;
        mimeType: string;
  ...
  private async buildProjectContext(
    ...
    const reviewFromDb = taskRows
      ...
      .map(
        (task) =>
          task.status_reason || `Task "${task.title}" is pending review.`,
      );

    const globalMemories = await this.memoryStore.loadMemories(
      "agency:global",
      40,
    );
    const reviewFromMemories = globalMemories
      .filter((entry) =>
        /\breview|revision|rework|acceptance\b/i.test(entry.content),
      )
      .slice(0, 4)
    ...
    if (!artifactSummary && this.completedTasks.length > 0) {
      artifactSummary = this.completedTasks
        ...
        .map(
          (item) =>
            `- ${item.task} (${this.summarizeArtifactPath(item.summaryPath)})`,
        )
        .join("\n");
    }
    ...
    const projectBrief =
      briefRaw ||
      `Project "${projectSlug}" is active. Use assigned task + acceptance criteria only.`;
    const resolvedActiveTask =
      activeTaskRow?.title || activeTask || "No active task assigned.";
    const recentReviewNotes = [...reviewFromDb, ...reviewFromMemories].slice(
      0,
      6,
    );
    ...
    return {
      ...
      artifactIndexSummary:
        artifactSummary || "No project artifacts indexed yet.",
      contextHash,
    };
  }

  private async persistProjectContextAudit(
    agentId: string,
    context: ProjectContextPayload,
  ): Promise<void> {
    const logDir = this.resolveWorkspacePath(
      path.posix.join("projects", context.projectSlug, "logs"),
    );
  ...
  private decisionReferencesProjectContext(
    decision: any,
    context: ProjectContextPayload,
  ): boolean {
  ...
  private async routeTaskBackToReview(
    agentId: string,
    reason: string,
    taskTitle: string,
  ) {
    await this.memoryStore.upsertTaskStatusByTitle(
      agentId,
      taskTitle,
      "review",
      reason,
  ...
  private normalizeMailAttachments(raw: any): MailAttachment[] {
    ...
    return raw
      ...
      .filter((item: MailAttachment | null): item is MailAttachment =>
        Boolean(item),
      );
  }
  ...
  private pushMail(message: MailMessage) {
    ...
    if (this.mailMessages.length > 250)
      this.mailMessages = this.mailMessages.slice(-250);
  ...
  public broadcastRuntimeQueueState(payload: {
    queued: number;
    running: number;
    workerSlots: Array<{
      id: string;
    ...
    this.broadcast("runtime:queue-state", payload);
  }

  public broadcastRuntimeRunState(payload: { run: any; steps?: any[] }) {
    this.broadcast("runtime:run-state", payload);
    if (payload?.run) {
      this.broadcast("task-update", {
        id: payload.run.id,
        task: payload.run.title,
        status: payload.run.status,
        statusReason: payload.run.errorMessage || payload.run.status,
        assigned_to: payload.run.assignedWorkerSlotId || "",
      });
    }
  }
  ...
  private configureInferenceProvider() {
    ...
    if (provider === "claude" || provider === "openai-compatible") {
      ...
      console.warn(
        "[Inference] Claude requested but no API key found; falling back to local Ollama.",
      );
    }
  ...
  async onCreate(options: any) {
    ...
    const dbPath =
      process.env.OFFICE_MEMORY_DB_PATH ||
      process.env.DATABASE_URL ||
      "./data/office-memory.db";
    ...
    const setupCoreAgent = async (
      ...
      const agencyMemories = await this.memoryStore.loadMemories(
        "agency:global",
        20,
      );
      const mergedMemories = [...previousMemories, ...agencyMemories].slice(
        -50,
      );
      if (mergedMemories.length > 0) {
        coreAgent.loadMemories(mergedMemories);
        console.log(
          `[${name}] Loaded ${previousMemories.length} personal + ${agencyMemories.length} agency memories from previous sessions`,
        );
      }
    ...
    const READ_ONLY: Capability[] = [
      ...
      {
        name: "list_files",
        description: "List files in the workspace (optionally recursive)",
      },
      {
        name: "stat_file",
        description: "Get file metadata such as size and timestamps",
      },
      {
        name: "read_file_chunk",
        description: "Read a chunk of a file for large files",
      },
      { name: "web_search", description: "Search the web for information" },
      {
        name: "fetch_url",
        description: "Fetch and read the visible text of a public URL",
      },
    ...
    const BUILDER: Capability[] = [
      ...
      },
      {
        name: "run_command",
        description:
          "Run an allowlisted shell command (ls, git status, docker ps, etc.)",
      },
      {
        name: "code_execute",
        description: "Execute a small JavaScript snippet",
      },
      { name: "web_search", description: "Search the web for information" },
      {
        name: "fetch_url",
    ...
    const VALIDATOR: Capability[] = [
      ...
      {
        name: "read_file_chunk",
        description: "Read large criteria/artifact chunks",
      },
      {
        name: "list_files",
        description: "Inspect project files and supporting evidence",
      },
      {
        name: "write_note",
        description: "Write review notes and revision guidance",
      },
    ];
...
Paired buddy: DevOps Automator. Flag issues early and recommend concrete fixes.`,
    ...
    await setupCoreAgent(
      ...
      [
        {
          name: "read_file",
          description: "Read existing page content and context files",
        },
        {
          name: "write_note",
          description: "Save keyword research and recommendations",
        },
        {
          name: "web_search",
          description: "Research keywords and competitor pages",
        },
        {
          name: "fetch_url",
          description: "Fetch page content to audit for SEO",
        },
...
Paired buddy: Proposal Strategist.`,
      ...
      [
        ...
        {
          name: "fetch_url",
          description: "Read a prospect website or public page",
        },
      ],
    );
...
Paired buddy: Sales Outreach.`,
      ...
      [
        ...
        {
          name: "web_search",
          description: "Research pricing or scope comparables",
        },
        {
          name: "fetch_url",
          description: "Read a prospect website, case study, or benchmark page",
        },
      ],
    );
    ...
    try {
      ...
      if (restored > 0) {
        console.log(
          `[Approvals] Restored ${restored} pending approval(s) from previous session.`,
        );
      }
    } catch (err) {
    ...
    this.onMessage("chat", (client, message) => {
      ...
      const attachments = this.parseChatAttachments(
        message?.attachments,
        "User",
      );
      console.log(
        `Chat from ${client.sessionId}: ${text} (attachments=${attachments.length})`,
      );
      this.broadcast("chat", {
        sender: "User",
        text,
    ...
    this.onMessage("start-project", async (client, message) => {
      try {
        ...
        client.send("start-project:result", {
          ok: false,
          error: error?.message || "Unable to start project.",
        });
      }
    });

    this.onMessage("runtime:subscribe", async (client) => {
      try {
        const runs = await this.memoryStore.listTaskRuns({ limit: 100 });
        client.send("runtime:runs-sync", { runs });
      } catch {
        client.send("runtime:runs-sync", { runs: [] });
      }
    });
    ...
    this.onMessage("tool:stat_file", async (client, message) => {
      const result = await this.toolExecutor.execute("stat_file", {
        path: message?.path,
      });
    ...
    this.onMessage(
      "file-share",
      async (client, message: SharedFileUpsertPayload) => {
        const nowIso = this.state.officeTime || new Date().toISOString();
        const fileId =
          message?.id ||
          `file_${Date.now()}_${Math.floor(Math.random() * 1000)}`;
        const actor = String(
          message?.createdBy || client.sessionId || "unknown",
        );
        const status: SharedFileStatus = message?.status || "shared";
        const filePath = String(message?.path || "").trim();
        const resolvedName = String(
          message?.name || path.basename(filePath || ""),
        );
        const resolvedMimeType = String(
          message?.mimeType || this.getMimeType(resolvedName || filePath),
        );
        ...
        const record: SharedFileRecord = {
          ...
          createdBy: actor,
          sharedWith: Array.isArray(message?.sharedWith)
            ? message.sharedWith
            : [String((message as any)?.audience || "agent")],
          status,
          createdAt: nowIso,
          updatedAt: nowIso,
        };
        ...
        await this.triggerArtifactValidation({
          projectId: String(
            message?.projectId || this.currentScenario || "general",
          ),
    ...
    this.onMessage("file-preview", async (client, message) => {
      const filePath = String(message?.path || "").trim();
      if (!filePath) {
        client.send("file-error", {
          message: "Path is required for file preview.",
        });
        return;
      }
      try {
    ...
    this.onMessage("file-open", async (client, message) => {
      ...
      const reviewHistory = file
        ? await this.memoryStore.listReviews({
            artifactId: file.id,
            limit: 100,
          })
        : [];
    ...
    this.onMessage("file-status-update", async (client, message) => {
      ...
      const allowed: SharedFileStatus[] = [
        "draft",
        "shared",
        "needs_review",
        "approved",
      ...
      if (!fileId || !allowed.includes(nextStatus)) {
        client.send("file-status-update-error", {
    ...
    this.onMessage("artifact-review-history", async (client, message) => {
      ...
      const history = await this.memoryStore.listReviews({
        artifactId,
        limit: 200,
      });
    ...
    this.onMessage("mail-send", async (client, message) => {
      const toAgentId = String(message?.toAgentId || "")
        .trim()
        .toLowerCase();
      ...
      if (!targetAgent) {
        client.send("mail-error", {
          message: `Unknown agent: ${toAgentId || "(empty)"}`,
        });
        return;
      }
      if (!body && attachments.length === 0) {
        client.send("mail-error", {
          message: "Add instructions or attachments before sending.",
        });
        return;
      }
      ...
      const ackBody = [
        ...
        body
          ? `Planned action: ${body.slice(0, 140)}`
          : "Planned action: reviewing provided context.",
        attachments.length > 0
  ...
  async update(delta: number) {
    if (Math.random() < 0.02) {
      console.log(
        `[Server] Agents: ${this.state.agents.size} | Session: ${this.sessionId}`,
      );
    }
    ...
    this.coreAgents.forEach((coreAgent, id) => {
      if (!this.thinkingLocks.get(id)) {
        ...
        this.coreAgents.forEach((other, otherId) => {
          ...
          if (otherState) {
            const dist =
              Math.abs(agentState.x - otherState.x) +
              Math.abs(agentState.y - otherState.y);
            nearbyAgents.push({
              name: other.config.name,
              role: other.config.role,
              distance: dist,
            });
          }
        });
        ...
        (async () => {
          ...
          return coreAgent
            ...
            .then(async (decision: any) => {
              ...
              if (
                coreAgent.currentTask &&
                !this.decisionReferencesProjectContext(decision, projectContext)
              ) {
                const reason =
                  "Guardrail: output did not reference active project/task context.";
                await this.routeTaskBackToReview(
                  id,
                  reason,
                  coreAgent.currentTask,
                );
              ...
              if (decision.action === "talk" && decision.message) {
                ...
                if (targetAgent) {
                  ...
                  /\b(user|ceo|faz)\b/i.test(targetName) ||
                  coreAgent
                    .getUnreadMessages()
                    .some((m: ConversationMessage) => m.from.includes("CEO"))
                ) {
                  this.broadcast("chat", {
                    sender: coreAgent.config.name,
              ...
              if (decision.action === "use_tool" && decision.toolCall) {
                ...
                if (decision.toolCall.name === "create_task") {
                  ...
                  const targetId =
                    assignee?.toLowerCase() || this.autoAssignAgent();
                  const targetAgent = this.coreAgents.get(targetId);
                  ...
                  decision.toolCall.name === "hire_agent" &&
                  id !== "ceo"
                ) {
                  ...
                  if (this.hireCount < 5 && !this.coreAgents.has(hireId)) {
                    ...
                    const hireAgent = new Agent({
                      ...
                      personality: {
                        ...
                        communicationStyle: hireRole.includes("Design")
                          ? "creative"
                          : "casual",
                        workHours: { start: "09:00", end: "17:00" },
                        breakFrequency: 90,
                  ...
                  this.createApproval({
                    ...
                    rationale:
                      decision.thought ||
                      "No rationale provided — please supply more context.",
                    isMajor: true,
                    pending: {
                  ...
                  const toolAuditLogId = await this.memoryStore.logToolAudit({
                    ...
                    toolName: decision.toolCall.name,
                    paramsHash: this.memoryStore.hashToolParams(
                      decision.toolCall.params,
                    ),
                    result: result.success
                      ? `success:${result.output.slice(0, 500)}`
                      : `failed:${(result.error || result.output || "").slice(0, 500)}`,
                  });
                  await this.captureTaskToolEvidence(
                    id,
                    decision.toolCall.name,
                    decision.toolCall.params,
                    result.success,
                  ...
                  if (
                    ...
                    String(
                      decision.toolCall.params?.status || "",
                    ...
                    await this.queueFileReviewApproval({
                      ...
                      summaryNote:
                        decision.toolCall.params?.summaryNote ||
                        decision.thought ||
                        "File requires CEO review.",
              ...
              if (Math.random() < 0.3) {
                ...
                await this.memoryStore.saveMemories(
                  id,
                  recentMemories,
                  this.sessionId,
                );
              ...
              await this.advanceTaskProgress(
                id,
                coreAgent,
                agentState,
                decision.action,
      ...
      this.state.agents.forEach((agent, key) => {
        ...
        if (!this.fastTrackMode && agent.action === "talk") {
          ...
          this.state.agents.forEach((other, otherKey) => {
            ...
            const dist =
              Math.abs(agent.x - other.x) + Math.abs(agent.y - other.y);
            if (dist < minDist) {
              minDist = dist;
  ...
  private handleSlashCommand(raw: string): boolean {
    ...
    if (cmd === "/project") {
      ...
      if (
        which.startsWith("xylon") ||
        which.startsWith("acme") ||
        which === ""
      ) {
        ...
        this.broadcast("chat", {
          sender: "🏢 Office",
          text: `Unknown project: ${argStr}. Try: /project xylon`,
        });
      }
      return true;
    }
  ...
  private validateAndNormalizeTaskTransition(params: {
    ...
    | { ok: false; status: TaskStatus; reason: string } {
  ...
  private async registerTaskTracking(
    agentId: string,
    taskTitle: string,
  ): Promise<void> {
    if (!taskTitle) return;
  ...
  private async getOrCreateTaskRecordId(
    agentId: string,
    taskTitle: string,
  ): Promise<string | null> {
  ...
  private async captureTaskToolEvidence(
    agentId: string,
    toolName: string,
    params: any,
    success: boolean,
  ...
  private async evaluateTaskEvidence(
    taskId: string,
  ): Promise<TaskEvidenceState> {
    ...
    for (const item of evidence) {
      ...
      if (
        ...
        item.evidenceType === "tool_execution" &&
        item.toolAuditLogId
      ) {
      ...
      if (
        !validatorApproved &&
        item.evidenceType === "validator" &&
        item.validatorDecision === "approved"
      ) {
  ...
  private taskStatusReason(evidence: TaskEvidenceState): string {
    ...
    if (evidence.artifactExists || evidence.toolExecutionSucceeded)
      return "pending_validation";
    return "waiting_for_artifact";
  }

  private async advanceTaskProgress(
    agentId: string,
    agent: Agent,
    agentState: any,
    action: string,
    ...
    const completion = await this.persistCompletedWork(
      taskTitle,
      agentId,
      agent.config.name,
    );
  ...
  private async persistCompletedWork(
    taskTitle: string,
    agentId: string,
    agentName: string,
  ...
  private startMeeting(topic: string, durationMs: number = 60_000) {
    ...
    this.broadcast("chat", {
      sender: "🏢 Office",
      text: `📣 CEO called a meeting: "${topic}". Everyone to the meeting table.`,
    });
    this.emitHighlight(
      "meeting",
      `Meeting called: ${topic}`,
      "All agents are heading to the meeting table.",
      "ceo",
    );
    this.broadcast("meeting-state", {
      active: true,
      topic,
      endsAt: this.meetingEndsAt,
    });
  ...
  private endMeeting() {
    ...
    this.broadcast("chat", {
      sender: "🏢 Office",
      text: `🔚 Meeting ended — back to desks.`,
    });
  ...
  private applyUserPriorityInstruction(text: string) {
    ...
    const targets = isBroadcast ? nameToId.map((n) => n.id) : matched;
  ...
  private isMajorToolCall(name: string, params: any): boolean {
    ...
    if (name === "run_command") {
      ...
      if (
        /\b(rm\s+-rf|shutdown|reboot|deploy|publish|release|drop\s+table)\b/.test(
          cmd,
        )
      )
        return true;
    ...
    if (params && (params.major === true || params.tag === "major"))
      return true;
    return false;
  }
  ...
  private createApproval(
    ...
    this.memoryStore
      .saveApproval(req)
      .catch((e) => console.error("saveApproval", e));
    ...
    this.emitHighlight(
      "approval",
      `Approval requested: ${req.requestedAction}`,
      req.rationale.slice(0, 120),
      req.requestedBy,
    );
  ...
  private async queueFileReviewApproval(input: {
    ...
    const fileId = String(
      input.fileId || safePath || `${input.sharedByAgentId}:${Date.now()}`,
    );
    const summaryNote = String(input.summaryNote || "No summary provided.");

    const existingPending = Array.from(this.approvals.values()).find(
      (approval) =>
        approval.status === "pending" &&
        approval.fileContext?.fileId === fileId,
    );
    ...
    if (existingPending) {
      ...
      await this.memoryStore
        .saveApproval(existingPending)
        .catch((e) => console.error("saveApproval(update)", e));
      await this.memoryStore
        .upsertSharedFile({
          id: fileId,
          filePath: safePath,
  ...
  private async persistRevisionNotes(
    projectId: string,
    taskTitle: string,
    artifactPath: string,
    reviewer: string,
  ...
  private async triggerArtifactValidation(
    ...
    if (!taskId) {
      ...
      if (inferredTask) {
        const found =
          await this.memoryStore.findMostRecentTaskByTitleAndAssignee(
            inferredTask,
            input.submitterId,
          );
    ...
    const existing = await this.memoryStore.listReviews({
      taskId,
    ...
    const rejectSignals =
      /\b(fail|missing|incomplete|rework|reject|blocked|error|todo)\b/;
    const decision: "approved" | "rejected" = rejectSignals.test(reviewSource)
      ? "rejected"
      : "approved";
    ...
    const nextStatus: TaskStatus =
      decision === "approved" ? "done" : "in_progress";
    await this.memoryStore.updateTaskStatus(taskId, nextStatus, notes);

    const taskHistory = await this.memoryStore.listReviews({
      taskId,
      limit: 100,
    });
    const artifactHistory = await this.memoryStore.listReviews({
      artifactId: input.artifactId,
      limit: 100,
    });
    ...
    this.broadcast("artifact-review-history", {
      artifactId: input.artifactId,
  ...
  private isWeakRationale(text: string): boolean {
    ...
    const reasonWords =
      /\b(because|so that|to|needed|prevents?|unblocks?|required|customer|client|deploy|fix|ship|launch|security|risk|revenue)\b/;
  ...
  private async resolveApproval(id: string, decision: "approved" | "rejected") {
    ...
    req.status = decision;
    this.memoryStore
      .updateApprovalStatus(id, decision)
      .catch((e) => console.error("updateApprovalStatus", e));
    ...
    if (linked) {
      const mappedStatus: SharedFileStatus =
      ...
      this.broadcast(
        "file-status-update",
        await this.memoryStore.getSharedFile(linked.id),
      );
    }
    ...
    this.emitHighlight(
      "approval",
      `CEO ${decision}: ${req.requestedAction}`,
    ...
    if (req.fileContext?.fileId) {
      this.memoryStore
        .updateSharedFileStatus(req.fileContext.fileId, decision, req.id)
        .catch((e) => console.error("updateSharedFileStatus", e));
      const requester = this.coreAgents.get(req.requestedBy);
      if (requester?.currentTask) {
        const taskRecordId = await this.getOrCreateTaskRecordId(
          req.requestedBy,
          requester.currentTask,
        );
        if (taskRecordId) {
          await this.memoryStore.addTaskEvidence({
            id: `tev_${randomUUID()}`,
          ...
          await this.advanceTaskProgress(
            req.requestedBy,
            requester,
            this.state.agents.get(req.requestedBy),
            "validation",
          );
        }
      }
    ...
    if (decision === "approved" && req.pending) {
      try {
        ...
        if (req.pending.toolName === "hire_agent" && requester) {
          ...
          const fakeDecision = {
            action: "use_tool",
            toolCall: { name: "hire_agent", params: req.pending.params },
            thought: "CEO approved",
          };
          ...
          this.broadcast("chat", {
            sender: "🏢 Office",
            text: `Hiring unblocked for ${req.requestedByName}.`,
          });
        } else {
          const result = await this.toolExecutor.execute(
            req.pending.toolName,
            req.pending.params,
          );
          const toolAuditLogId = await this.memoryStore.logToolAudit({
    ...
    setTimeout(() => {
      ...
      this.memoryStore
        .deleteApproval(id)
        .catch((e) => console.error("deleteApproval", e));
      this.broadcast("approvals-sync", this.listApprovals());
    }, 10000);
  }
  ...
  private updateAgentViralMetrics(agentId: string, action: string) {
    ...
    state.riskLevel = this.clamp01(
      state.riskLevel + (action === "use_tool" ? 0.02 : -0.004) + jitter,
    );
    state.mood = this.clamp01(
      state.mood + (action === "talk" ? 0.02 : -0.002) + jitter,
    );
    state.reputation = this.clamp01(
      state.reputation + (action === "work" ? 0.015 : 0.001) + jitter / 2,
    );
  }
  ...
  private applyScenarioKickoff(scenarioName: string) {
    ...
    if (
      normalized.includes("xylon growth sprint") ||
  ...
  private kickoffXylonGrowthSprint() {
    this.currentProjectSlug = "xylon";
    const project =
      "ACME Manufacturing — M365 Copilot rollout + oversharing remediation + landing page + SOW";
    const brief = [
      `NEW PROJECT: ${project}.`,
    ...
    this.emitHighlight(
      "scenario",
      "Xylon Growth Sprint kicked off",
      project,
      "shepherd",
    );
  ...
  private applyChaosEvent(eventName: string) {
    ...
    this.state.agents.forEach((agent, id) => {
      agent.mood = this.clamp01(
        agent.mood + selected.moodDelta + (Math.random() - 0.5) * 0.08,
      );
      agent.riskLevel = this.clamp01(
        agent.riskLevel + selected.riskDelta + Math.random() * 0.08,
      );
      agent.momentum = this.clamp01(
        agent.momentum + selected.momentumDelta + (Math.random() - 0.5) * 0.05,
      );
  ...
  public registerAudienceVote(eventName: string, voterId?: string) {
    ...
    const totalVotes = Object.values(this.audienceVotes).reduce(
      (sum, value) => sum + value,
      0,
    );
    const shouldTrigger =
      this.audienceVotes[normalized] >= 3 || totalVotes % 5 === 0;
  ...
  public getEpisodeRecap() {
    ...
    const leaderboard = Array.from(this.state.agents.entries())
      .map(([id, agent]) => {
        const impact =
          agent.momentum * 0.35 +
    ...
    const avgMomentum = leaderboard.length
      ? leaderboard.reduce((sum, item) => sum + item.momentum, 0) /
        leaderboard.length
      : 0;
    const avgRisk = leaderboard.length
      ? leaderboard.reduce((sum, item) => sum + item.riskLevel, 0) /
        leaderboard.length
      : 0;
    const outcome =
      avgMomentum > 0.65 && avgRisk < 0.5
    ...
    return {
      ...
      outcomeCard: {
        ...
        activeRelationships: Array.from(this.relationships.values()).filter(
